...
- **Stay signed in** — this device remembers the last unlocked profile. Reopening the app or browser does not ask for the passcode again. Switch profile locks it. Switching to a different passcode profile still asks for that code.
...
3. Start the gym. Either double-click **`Start-Gym.command`** in Finder, or:
   ...
   An iPad refresh does **not** pull GitHub. Safari keeps the last Class flow selected, so landing on Handstand Hold again is not proof the new code loaded. After each push: stop the gym window, then `npm run gym:mac` again. That script now parks gym data files (including IG / coach still JPEGs), resets this folder to GitHub `v2-rebuild`, reloads itself so the banner matches the files, and rebuilds the phone bundle. Dirty `data/` files will not block the update. The current stamp is **Firm build**. If you still see Print, Keep, Sweep, Path, Gleam, Sleet, Rime, Glaze, Frost, Ash, or Ember, this window is still old. `/api/health` should include `"holdBuild":"firm"`.

   Yesterday’s coach stills (mad cat, supports, lightning bolt, handstand variations, and the rest) now **ship in git** under `public/learn/coach-stills/` and `src/assets/coach-stills/`. `gym:mac` copies those JPEGs into `data/coach-blobs/` on the first stills request so a paused Blob or a wiped iPad cannot blank the cards. Coach add / replace / rename / delete of a still writes the gym file immediately and other devices pick it up on the next 4-second pull. Drag a photo onto a listed still in Learn to fill it without retyping the name. **Send everything on this device** also sends shape stills, not only roster faces.
