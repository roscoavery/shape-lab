...
3. On **either card** tap **Full screen with delay cam** / **Full screen with reference**. That opens **top / bottom** (reference starts larger). **Drag the bar between the two videos** to give reference or delay cam more of the window — the pictures stay `object-contain` and are not stretched. Switch to **Left / right** in the side rail if you want. **Min** on the reference (or **Delay cam** in the rail) makes delay cam full screen with the looping clip still in the corner; **Min** on delay cam does the reverse. Athlete performs with **Delay cam** at 6–20s. Use the bottom-right **Live** thumbnail to set the tripod, then go back to delay. Tap **Clip** next to **Show** to switch reference videos. Tap **Record** after the skill to save that buffer into the profile’s video library, or **Replay last Ns** to pause, play, and scrub, then **Save in app** or **Save to Photos**.
