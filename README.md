...
Side-by-side technique study: a **reference video** (the technique to copy) next to the **athlete camera** (live view, delay cam, or recorded replay). Stacks vertically on phones. **Full screen with reference** opens **top / bottom** by default — reference on top and larger, delay cam flush underneath with no gap between the two. Hide the side rail for a clean split. **Min** on either pane shrinks that view to a bottom-right chip so the other can go full screen — the chip keeps playing. Tap **Swap** or the chip to flip between a full replay and a full reference. On delay cam, a small **Live** picture sits in the corner so you can check tripod framing without leaving the delay; tap it (or **Live** on the minimized camera chip) to open live view. **Clip** next to **Show** on the reference picks another library clip. Shape overlay opens as a **large filmstrip** (not a tiny list); the selected still floats over both videos so you can drag it anywhere, then tap **×** to hide it.
...
3. On **either card** tap **Full screen with delay cam** / **Full screen with reference**. That opens **top / bottom** (reference starts larger). **Drag the bar between the two videos** to give reference or delay cam more of the window — the pictures stay `object-contain` and are not stretched. Switch to **Left / right** in the side rail if you want. **Min** on the reference (or **Delay cam** in the rail) makes delay cam / replay full screen with the looping clip still in the bottom-right chip; **Min** on delay cam or Replay Last does the reverse. Tap **Swap** (or the chip itself) to flip which one is full. Athlete performs with **Delay cam** at 6–20s. Use the **Live** thumbnail to set the tripod, then go back to delay. Tap **Clip** next to **Show** to switch reference videos. Tap **Record** after the skill to save that buffer into the profile’s video library, or **Replay last Ns** to pause, play, and scrub, then **Save in app** or **Save to Photos**.
