...
Profiles save on this gym computer and on the Shape Lab server (`/api/roster`), so a new Preview or phone link still has the roster. **Every athlete profile requires a passcode** (set when you create it, or the first time you open an older profile). Unlock that profile on any browser or link to load its homework, hold times, Compare URLs, and **video library**. Homework logs, Tasks progress, and saved hold times stay with the profile. While a profile is unlocked, IG / Compare URL lists save into that athlete’s library on the server. **Ryan** is always in the profile list and cannot be deleted. Creating the same name again selects the existing profile. Clips in the video library live on this gym computer (`/api/athlete-videos`) so every link can play them. There is no Instagram login that can post Stories for you from a website.
...
1. Create an athlete under **Athlete profile** and set a passcode (4+ characters).
