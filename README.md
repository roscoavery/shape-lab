...
3. Start the gym. Either double-click **`Start-Gym.command`** in Finder, or:
   ...
   **Version 4** (sign-in, private roster) is a different branch. A Cursor preview on this cloud machine is not reachable from your phone. To put V4 on the same HTTPS gym link, stop the old gym window and run:
   ...
   Then open **https://gym.shapelab.win** and sign in (or create the first gym admin account). Look for **Mark build**. More → Accounts copies a sign-in link; email is off until SMTP is set. `npm run gym:mac` without `:v4` stays on the working V3 / `v2-rebuild` gym. Athlete files in `data/` stay on this Mac either way.
