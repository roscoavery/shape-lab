...
- **IG shapes library** (Learn) stores every Screenshot / Shot crop from Compare, Learn scroll, and reels. Ryan can move a crop to another shape, rewrite its name, and pin it into the main Shape library as a coach still. Deletes stay gone. Stills persist on the gym URL so they are not stuck on one iPad.
...
3. Start the gym. Either double-click **`Start-Gym.command`** in Finder, or:
   ...
   An iPad refresh does **not** pull GitHub. Safari keeps the last Class flow selected, so landing on Handstand Hold again is not proof the new code loaded. After each push: stop the gym window, then `npm run gym:mac` again. That script now parks gym data files, resets this folder to GitHub `v2-rebuild`, and rebuilds the phone bundle. Dirty `data/` files will not block the update. The current stamp is **Keep build**. If you still see Sweep, Path, Gleam, Sleet, Rime, Glaze, Frost, Ash, or Ember, this Mac did not pick up the new files. `/api/health` should include `"holdBuild":"keep"`.
