...
Create an athlete profile and attach an Instagram `@handle` on the athlete card. **Ryan is always in the profile list** and cannot be deleted. Profiles save on this gym computer and on the Shape Lab server (`/api/roster`), so a new Preview or phone link still has the roster. While **Ryan** is selected, IG crops from Compare save into the app (`/api/ig-stills`) so they are not stuck in one browser. Creating the same name again selects the existing profile. Clips stay on this device in IndexedDB. There is no Instagram login that can post Stories for you from a website.
