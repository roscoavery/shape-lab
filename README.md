...
Profiles save on this gym computer and on the Shape Lab server (`/api/roster`), so a new Preview or phone link still has the roster. **Every new athlete sets a 4-digit passcode on Create.** Unlock that profile on any browser or link to load its homework, hold times, Compare URLs, and **video library**. The app does not ask you to type a passcode until that profile already has one. **Ryan’s passcode is 2223.** Homework logs, Tasks progress, and saved hold times stay with the profile. While a profile is unlocked, IG / Compare URL lists save into that athlete’s library on the server. **Ryan** is always in the profile list and cannot be deleted. Creating the same name again selects the existing profile. Clips in the video library live on this gym computer (`/api/athlete-videos`) so every link can play them. There is no Instagram login that can post Stories for you from a website.
