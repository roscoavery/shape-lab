...
Profiles save on this gym computer and on the Shape Lab server (`/api/roster`), so a new Preview or phone link still has the roster. **Every new athlete sets a 4-digit passcode on Create.** Unlock that profile on any browser or link to load its homework, hold times, and **video library**. The app does not ask you to type a passcode until that profile already has one. **Ryan’s passcode is 2223.** Homework logs, Tasks progress, and saved hold times stay with the profile. **Compare URLs are gym-wide** — every athlete and every browser sees the same library. Unlock **Ryan** to add, rename, or reorder clips, then tap **Save into the app** (adds also auto-save) so the list is written into `data/library.json` for every link. **Ryan** is always in the profile list and cannot be deleted. Creating the same name again selects the existing profile. Clips in the video library live on this gym computer (`/api/athlete-videos`) so every link can play them. There is no Instagram login that can post Stories for you from a website.
...
- **Record** (on delay cam) — tap **after** the skill. That flushes the rolling buffer of what just happened (not a live-forward take) and saves it into the unlocked athlete’s **video library** (Athletes tab, grouped by date). Unlock a profile first.
- **Replay last Ns** — tap **Replay last 6s** (or whatever the buffer slider is set to) to open a real player of that stretch: pause, play, scrub, slow-mo. **Line** is the default markup on the picture (tap, tap, tap). **Draw** is freehand. **Arrow** is press, drag, and let go. Those tools stay on the video; they do **not** cover Pause, the playhead, **0.25x / 0.5x / 1x**, or **A / B** loop points. Set **A** and **B** at the current time to loop a phase (Clear A/B to drop it). **Screenshot** is press one corner, drag to the opposite corner, pick a listed shape — or **None** — or type a custom name, and save into **Learn → IG shapes**. **Clear** wipes marks. The same markup is on the looping reference, including full-screen split. **Save in app** keeps it in Recorded attempts (IndexedDB) and, with a profile unlocked, also the video library. **Save to device** downloads the file.
...
2. Wait for it to load, then set **A** and **B** around the key phase; slow to 0.25x or 0.5x. Pause, the playhead, and speed stay usable while Line is on.
