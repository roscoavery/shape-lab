...
- Save attempts and athlete profiles in the browser (`localStorage`)
...
src/
  ...
  lib/storage.ts        localStorage athletes & attempts
  components/           camera, scores, profiles, sequences
  App.tsx               main UI
```
...
- Tailwind CSS v4
- `@mediapipe/tasks-vision` Pose Landmarker (lite model from Google CDN)

## Roadmap (architecture hooks)

Cartwheel gaze/hands, roundoff segmentation, rolls, V-ups, drills library, progression roadmaps, education pages, athlete folders/groups, parent sharing — not built yet; shape/sequence config is designed so new shapes drop in without rewriting the app.
