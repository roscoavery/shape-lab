...
- **Wins** a separate spam-friendly feed for firsts and small hits, with a **big win** checkbox to also post on the gym feed. Attach a clip from Photos on the class clock, the Wins composer, or **Add clip** on a win that was already posted. Like, Repost, and High five flip immediately; **All 3** does all of them at once. When a coach logs a win it still belongs to the athlete, with **shared by Coach Ryan** (or whichever coach posted it).
- **Stay signed in** — Version 4 requires an email + password account session for private gym data. After that, this device can remember the last unlocked profile. The same login stays signed in on multiple phones and iPads up to a **device limit** you set in More → Accounts (1–12). Signing in past that number drops the oldest device. Changing your password still signs the others out. The old 4-digit PIN only picks a profile on this device; it is not enough to download the roster. Switch profile locks the PIN. Sign out ends the account session.
- **My profile** (Today) — you (or gym admin) can add or take a photo, pinch-zoom a small face, and save. Only the owner and gym admin can edit a profile photo or answers. Signed-out visitors cannot. Twist, dominant hand, and skate stance sync into Research.
...
- **Parents** select their athlete on Profiles. Coaches see that link. Parents can open that kid’s wins, homework, lessons, upcoming matched calendar lessons, recent attendance, athlete-visible coach notes, and improving hold times without unlocking the child’s passcode. They can also **add and log homework** on that athlete.
...
- **Calendar** (Today) — month, week, or day against iCloud. Week view is a Fantastical-style grid: hour labels, seven day columns, an all-day row, and colored blocks that split when events overlap. On a phone, the wide grid is contained inside the Calendar card and leftover days scroll sideways without widening or trapping the whole page; **Full screen** opens the same grid edge to edge. Calendar events matched to an athlete appear on that athlete’s and parent’s Home desk with the coach and scheduled time.
- **Class clock** (Today) — swipe left from core holds to V-ups / push-ups, then wall sit / calf / nordic / glute bridges, then slantboard squats / Poliquin steps / tib raises / split squats. Hollow looks like the other hold buttons, with Arms up / Arms down in a smaller line underneath. Push-ups take elbows-in, wide hands, piked, or handstand. Split squats take an optional elevated front foot plus a height slider from flat ground to just above the knee. Logs go onto selected athletes as **in class**.
...
- **Gym desks** — every profile defaults to **Tumble Smart Athletics**. A coach’s Today list is that gym plus athletes who take class there or who they do privates with. Camp / clinic groups stay off the main desk until you tap **Add to this gym**. **Add to camp** puts a Tumble Smart athlete on a clinic list without taking them off this gym. **Search all** or pick another gym to find anyone; other-gym names are labeled. Network still connects everyone. Profiles, photos, and class-station answers sync on the gym URL so a new profile on the iPad shows on every phone. Gym admin can **Look as** coach, gym owner, parent, or athlete from the header or Accounts without extra logins. The large profile directory is collapsed by default and scrolls inside the phone when opened.
- **Lessons** — write a plan (holds, drills / exercises, skills to work, Compare, talk) and drag to reorder. Those notes sit at the top of the live lesson. Start / end times can be typed during the lesson or on the recap. Each recap note is marked **Athlete can see** or **Coach only**. Athletes and parents only see the athlete notes. Leaving the app does not end the lesson — come back and hit End lesson. Recaps fold after a day into Older recaps. Remove hides a recap without deleting the notes; Show again brings it back.
- **IG shapes library** (Learn) stores every Screenshot / Shot crop from Compare, Learn scroll, and reels. Ryan can move a crop to another shape, rewrite its name, and pin it into the main Shape library as a coach still. Deletes stay gone. The JPEG lives in `data/ig-blobs/` on the gym Mac (and a copy stays on the phone). `gym:mac` parks those files before it updates, and `gym:pull` copies the pictures — not just the names — so an empty card cannot replace a real still.
- **Reference library deletes** stay gone across phones and the gym URL. A deleted landing drill cannot come back from the shipped seed.
- **Library play** — Instagram Reels resolve the same way Continue on web does, play in Shape Lab, then stay cached (this phone plus a gym copy) so the next open does not hit Instagram again. A broken resolve is forgotten and retried instead of being stuck. YouTube plays in an embed, not a raw `<video>` tag.
...
3. Start the gym. Either double-click **`Start-Gym.command`** in Finder, or:
   ...
   **Version 4** (sign-in, private roster) is a different branch. A Cursor preview on this cloud machine is not reachable from your phone. To put V4 on the same HTTPS gym link, stop the old gym window and run:
   ...
   Gym-added shapes (the ones with a **Gym** badge in Learn) can be deleted by a signed-in coach on the shape card or detail page. That delete is written into `data/coach-content.json` and stays gone when phones sync. Shipped curriculum shapes cannot be deleted. To list or remove gym-added shapes from Terminal on this Mac, after `gym:mac`:

   ```bash
   npm run gym:shapes
   npm run gym:shapes -- "Exact shape name"
   ```
...
4. Ctrl+C that window, then `npm run gym:mac` again. Use only the https line from the **new** window.
...
Learn home is a magazine page: a still mosaic, then Shape test, Shape test 2 (body positions), and Movements. **Shape test 2** asks where the hips and hands actually sit (starting lunge vs mountain climber). **Movements** names joint actions — wrist extension is also wrist dorsiflexion; pointing toes is plantarflexion. Coach study splits anatomy and progressions into sectioned mixes so the same Q&A does not come back every time.
...
On top of that, the **coach can assign** any shape from the library as homework, a **class flow**, or a **drill** that Ryan linked to a shape. Sequence homework opens **Practice → Class flows** on that assigned task — same spoken guidance as class. Full screen shows a big **Start** on the camera. The coach still drags to any corner; the delay-cam corner pip stays hidden in class-flow fullscreen (the rolling buffer still records the replay). After you watch the run and read the grades, pick whether to keep the clip — **Photos**, the **video library**, **My shapes**, or dump it. Each finished run logs how many times they did the sequence and the overall score on the homework card. Linked drills play the video. Athletes can also self-select ("Coach assigns" / "Athlete picks" when adding). Ryan can add extra coach stills on a shape page and pick which one is the main still.
...
4. **IG shapes library** — stills cropped from Compare. On a looping Instagram clip or replay, tap **Screenshot**, press one corner of the shape, drag to the opposite corner, tag a listed shape or type a **custom name** if it is not in the list, and Save. Select the **Ryan** profile first if you want that still saved **into the app** (this gym computer) so every browser and phone link still has it. Other profiles keep crops on this device only. These never replace shipped coach stills unless Ryan pins one with **Show in the main Shape library**. On **Tasks**, **Homework**, **Coach**, or **Compare**, **Still overlay** lets you scroll left/right through Shape library or IG shapes and put any picture on the live camera or Compare video (opacity slider).
...
- **Record** (on delay cam) — start / stop of the **buffered camera picture**, like a screen record of that pane (not the phone’s screen, not a flush of the rolling buffer). Tap Record to start, tap Stop when you are done. The clip is kept in the app, offered to **Photos / Files**, and with a profile unlocked it also lands in the video library. **Replay last** can swap with the reference still the same way the live delay feed can.
...
4. On **Today**, the **Calendar** section (collapsed until you open it) has month, week, and day. Week and day show an hour timeline of events. Event notes stay collapsed until you tap Details. Sync, unlock with the coach passcode if asked, and **New event** writes back to iCloud. A lesson that is happening now glows **Start lesson** and preselects the matched athlete. Failed syncs do not block manual **Start lesson**.
