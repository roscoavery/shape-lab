...
3. On **either card** tap **Full screen with delay cam** / **Full screen with reference**. The rail starts closed; **Controls** sits under **Clear**. **Drag the bar** in split to resize. **Min** parks a pane in a corner chip you can drag; **Swap** flips full vs chip; **Split** on the chip restores top / bottom. Athlete performs with **Delay cam** at 6–20s. Tap the **Live** thumbnail to set the tripod, then **Tap to close** to return to delay. Delay **EXIT** returns to live camera without leaving Compare. Tap **Clip** next to **Show** to switch reference videos. Tap **Record** after the skill to save that buffer into the profile’s video library, or **Replay last Ns** to pause, play, and scrub, then **Save in app** or **Save to Photos**.
