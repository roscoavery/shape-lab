...
if [ -d .git ]; then
  ...
  echo
  echo "============================================================"
  echo "  PINK BUILD   $(git rev-parse --short HEAD)   $(git log -1 --pretty=%s)"
  echo "  iPad must show a bright PINK bar that says Pink build."
  echo "  Aqua, orange, or no bar means this window is still old —"
