/**
 * SHA-256 for browsers where crypto.subtle is missing.
 * http://192.168… is not a secure context, so Web Crypto throws and gym boot
 * used to sit on “Loading this gym” forever.
 */
...
export function sha256Bytes(message: Uint8Array): Uint8Array {
  ...
  const bytes = new Uint8Array(message.length + 1 + padLen + 8)
  bytes.set(message)
...
export function sha256Hex(message: Uint8Array): string {
  return [...sha256Bytes(message)].map((b) => b.toString(16).padStart(2, '0')).join('')
}
