...
export function TodayCalendarSection({ coachId, athletes, onStartLesson }: Props) {
  ...
  return (
    <section className="min-w-0 max-w-full overflow-x-hidden rounded-xl border border-[var(--panel-border)] bg-[var(--panel)] p-4">
          ...
          const matchLine = ev.matchedAthleteId
            ...
            : ev.matchStatus === 'ambiguous'
              ...
              : ev.matchStatus === 'needs_athlete'
                ...
                : 'Coaching event'
          return (
            <li key={ev.id} className="rounded-lg bg-[#121820] px-3 py-2">
              <div className="flex flex-wrap items-start justify-between gap-2">
                <button
                  type="button"
                  className="min-w-0 flex-1 text-left"
                  onClick={() => setOpenEventId((cur) => (cur === ev.id ? null : ev.id))}
                >
                  <p className="text-xs text-[var(--muted)]">{formatTimeRange(ev.startAt, ev.endAt)}</p>
                  <p className="font-medium">{ev.title || 'Untitled'}</p>
                  <p className="text-xs text-[var(--muted)]">{matchLine}</p>
                </button>
                {renderActions(ev)}
              </div>
              {open && (
